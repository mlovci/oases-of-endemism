...
However, we distinguish here between **linear scale-standardization** (z-scoring) and **modeling non-linear ecological structures and thresholds**. While PCA identifies orthogonal axes of maximum *linear* variance, it is structurally unable to model non-linear relationships, unimodal tolerances, and zero-inflation. By introducing non-linear manifold learning (t-SNE), Factor Analysis (FA), count-appropriate Poisson GLMs, and bootstrap Random Forest regressions, our reanalysis uncovers **six key differences in ecological and conservation conclusions** compared to the published work:
...
4. **Actionable Depth Threshold Targets (Partial Dependence Plots)**:
    * *Published Conclusion*: Pool depth is identified generally as a significant factor in PCA biplots.
    * *Extended Conclusion*: Our non-parametric Partial Dependence Plots (PDPs) and LOWESS curves reveal a sharp, non-linear **threshold boundary at ~20–30 cm** for water depth. Expected endemic richness remains constrained and low below this range, but increases exponentially above it, giving managers a specific, actionable physical target for excavation and water-rights protection.
5. **Environmental vs. Biological Distinctiveness (The Oasis Niche)**:
    * *Published Conclusion*: Regional aquifer springs differ from other springs primarily due to biological factors (presence of endemics) and overlap environmentally with local hot springs.
    * *Extended Conclusion*: Our non-linear t-SNE manifold learning reveals that regional aquifer springs form a tight, completely isolated cluster. Because t-SNE groups points based on local similarity in the high-dimensional space, this tight clustering demonstrates that their biological uniqueness is a direct product of their unique, rare environmental niche (the rare intersection of stable warm temperature, high depth, high bank cover, and coarse substrates), rather than overlapping generally with geothermal springs.
6. **Scale-Dependent Species Co-occurrence and Management Focus**:
    * *Published Conclusion*: Springs generally group together, suggesting a uniform positive correlation structure among all native taxa.
    * *Extended Conclusion*: Species correlations undergo a scale-dependent reversal. Globally, all native species correlate positively due to a massive landscape-level presence/absence gradient (hotspots vs. dry sites). Regionally within oases, however, springsnails are decoupled from other endemics by siltation sensitivity, and non-natives cluster closely with natives due to shared physical stability. Consequently, conservation cannot rely on a single global "biodiversity hotspot" protection plan, but must target taxon-specific environmental filters (such as substrate desiltation for springsnails).
...
*   **Multivariate Resemblance and Distance-Based Models (DistLM & SIMPER)**: In community ecology, analyzing how multi-species assemblages shift along environmental gradients is typically modeled using multivariate distance-based resemblance matrices (e.g., Bray-Curtis) related to environmental factors via Distance-based Linear Models (DistLM) and Similarity Percentages (SIMPER) (Legendre & Anderson 1999; Anderson 2001; Clarke 1993). This community-level focus captures the shared responses of the entire biota as a whole rather than modeling individual species in isolation.
*   **Non-Parametric and Permutation-Based Methods**: Species richness distributions across a large landscape of desert springs are typically highly skewed and zero-heavy, as the vast majority of ephemeral cold springs do not sustain endemics. Given this severe zero-inflation, rank-based non-parametric tests (Kruskal-Wallis, Spearman rank correlation) and permutation-based multivariate regressions (DistLM) are mathematically robust choices that avoid violating the assumptions of normality and homoscedasticity inherent in standard parametric linear regression (Legendre & Legendre 2012).
...
3. **The Manifold of Endemism (t-SNE)**: A global t-SNE projection colored by endemic richness reveals that endemic hotspots cluster tightly together. Because t-SNE groups points based on local similarity in the underlying 15-dimensional space, this tight clustering shows that hotspots are restricted to a highly specific, rare combination of environmental conditions. This highlights how narrow and distinct the environmental niche of these oases is (see Figure 5).
...
**Ecological Interpretation**: The t-SNE manifold displays a clear separation of springs. The vast majority of desert springs (Local Cold, Local Hot) form a large, diffuse cloud with zero or near-zero endemic species. The high-endemic springs (mostly Regional Aquifer springs) form a tight, distinct, and highly localized cluster in the t-SNE space. Because t-SNE maps points based on multi-dimensional similarity, this tight clustering demonstrates that the environmental conditions required to preserve high levels of endemism are extremely specific, narrow, and ecologically rare (reflecting a highly restricted, distinct region in the 15-dimensional environmental parameter space). These conditions cannot be easily replicated or found in standard local runoff springs.
...
*   **Model Selection and Goodness-of-Fit Comparisons**:
    ...
    - **Concordance of Variable Selection**:
        ...
        This indicates that whether modeling the community structure multivariable (via DistLM distance matrices) or modeling taxon-specific richness parametrically (via Poisson GLMs), the same core physical, chemical, and disturbance features emerge as the drivers of Great Basin desert spring endemism.
